...
fn test_register_key() {
    new_test_ext().execute_with(|| {
        let pk = [1u8; 1952];
        assert_ok!(PqDilithium::register_key(RuntimeOrigin::signed(1u64), pk));
...
fn test_revoke_key() {
    new_test_ext().execute_with(|| {
        let pk = [1u8; 1952];
        assert_ok!(PqDilithium::register_key(RuntimeOrigin::signed(1u64), pk));
        assert_ok!(PqDilithium::revoke_key(RuntimeOrigin::signed(1u64), pk));
...
fn test_duplicate_key_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PqDilithium::register_key(RuntimeOrigin::signed(1u64), pk));
        assert_noop!(
            PqDilithium::register_key(RuntimeOrigin::signed(1u64), pk),
...
fn test_revoke_nonexistent_key_fails() {
    new_test_ext().execute_with(|| {
        let pk = [1u8; 1952];
        assert_noop!(
            PqDilithium::revoke_key(RuntimeOrigin::signed(1u64), pk),
